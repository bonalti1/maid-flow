...
1. GHL → Automations → Create Workflow → trigger **Inbound Webhook** → copiar la
   URL.
2. Admin del motor → cuenta Maid Flow Ventas → botón **🤖 GHL** → pegar la URL
...
**Payload que manda el motor:** `{source:"maid-flow",
...
> Eres el asistente de Maid Flow. El equipo YA tiene el teléfono de la clienta y
> le va a MARCAR — tu único trabajo es mantenerla interesada hasta esa llamada.
> REGLAS: (1) Solo puedes afirmar esto: Maid Flow le da a la limpiadora su propia
