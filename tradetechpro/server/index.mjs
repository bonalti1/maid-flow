...
body{background:#084563;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}
...
input:focus{border-color:#0C5A8A}
button{width:100%;margin-top:10px;padding:14px;border:none;border-radius:12px;background:#0C5A8A;color:#fff;font-size:16px;font-weight:800;cursor:pointer}
...
<style>body{font-family:Inter,Arial,sans-serif;max-width:560px;margin:40px auto;padding:0 16px;color:#0C5A8A}h2{margin-bottom:6px}
.link{background:#F7EFD8;border:2px solid #1F9BD2;border-radius:12px;padding:14px;word-break:break-all;font-size:14px;margin:14px 0}
a{color:#1F9BD2;font-weight:800}</style></head><body>
...
<style>body{font-family:Inter,Arial,sans-serif;max-width:560px;margin:40px auto;padding:0 16px;color:#0C5A8A}
.link{background:#F7EFD8;border:2px solid #1F9BD2;border-radius:12px;padding:14px;word-break:break-all;font-size:14px;margin:14px 0}
a{color:#1F9BD2;font-weight:800}</style></head><body>
...
body{background:#F5F6F8;color:#084563;letter-spacing:-0.011em}
...
header b{font-size:16px;font-weight:700;letter-spacing:-0.02em}header b em{color:#3FB6E8;font-style:normal}
...
.chart .num{font-size:11px;font-weight:800;color:#084563}
...
td{padding:14px 10px;border-bottom:1px solid #F2F4F7;font-weight:600;color:#0E5180;vertical-align:middle}
...
.newform input:focus{border-color:#3FB6E8;box-shadow:0 0 0 4px rgba(248,180,8,.18)}
.newform button{background:#3FB6E8;color:#0C5A8A;border:none;border-radius:13px;padding:13px 24px;font-weight:700;cursor:pointer;font-size:14.5px;transition:transform .12s,filter .15s;box-shadow:0 6px 16px rgba(248,180,8,.3)}
...
.seg.on{background:#fff;color:#0C5A8A;box-shadow:0 1px 3px rgba(16,27,48,.12)}
.segcustom{display:inline-flex;gap:7px;align-items:center}
.segcustom input{font-family:inherit;padding:8px 10px;border-radius:10px;border:1px solid #E4E7EC;font-size:13px;font-weight:600;color:#0E5180;outline:none}
.segcustom input:focus{border-color:#3FB6E8;box-shadow:0 0 0 3px rgba(248,180,8,.18)}
.segcustom button{background:#0C5A8A;color:#fff;border:none;border-radius:10px;padding:9px 16px;font-weight:700;font-size:13px;cursor:pointer}
.segcustom button.on{background:#3FB6E8;color:#0C5A8A}
...
.sub.gold .v{color:#3FB6E8}.sub.gold .l{color:#9DA8C4}
...
.lprev{width:134px;height:86px;border-radius:10px;overflow:hidden;border:1px solid #E4E7EC;flex-shrink:0;background:#084563;box-shadow:0 4px 12px rgba(16,27,48,.08)}
...
<header><b style="color:#fff;font-weight:900">Maid<span style="color:#3FB6E8">Flow</span></b><b>· Admin</b><span class="tag"><a href="/admin/economics" style="color:#3FB6E8">🧮 Calculadora</a> · <a href="/cs" style="color:#9DA8C4">🎧 Servicio</a> · <a href="/admin?logout" style="color:#9DA8C4">salir</a></span></header>
<div class="wrap">
...
<div class="panel">
  ...
  ${[
    ...
    ${links.map(([name, url]) => {
      ...
      <span class="lbtns">
        <button onclick="cpy(this,'${url}')" style="background:#3FB6E8;color:#0C5A8A;border:none;border-radius:8px;padding:7px 12px;font-weight:800;cursor:pointer;font-size:12px">Copiar</button>
        <a href="${url}" target="_blank" style="background:#0C5A8A;color:#fff;border-radius:8px;padding:7px 12px;font-weight:800;text-decoration:none;font-size:12px">Abrir</a>
      </span>
    </div>`; }).join("")}
  `).join("")}
...
<div class="panel">
  ...
  ${list.map((c) => {
    ...
    return `<tr data-name="${esc(c.name).toLowerCase()} ${c.slug}">
      <td><a href="/admin/c/${c.slug}" style="color:#0C5A8A;font-weight:800">${esc(c.name)}</a>${isB ? ' <span class="pill gold">interno</span>' : ""}</td>
      <td>${isPaused
...
.appheader b{font-size:16px;font-weight:700;letter-spacing:-0.02em}.appheader b em{color:#3FB6E8;font-style:normal}
...
.card.gold .v{color:#3FB6E8}.card.gold .l{color:#9DA8C4}.card.gold .s{color:#9DA8C4}
...
.fld input[type=range]{flex:1;accent-color:#3FB6E8}
.fld .val{min-width:78px;display:flex;align-items:center;background:#F4F6FA;border:1px solid #E4E7EC;border-radius:9px;padding:6px 9px;font-weight:800;font-size:13.5px;color:#0C5A8A}
.fld .val .pre{color:#9097A3;font-weight:700;margin-right:2px}
.fld .val input{width:100%;border:none;background:none;outline:none;font-weight:800;font-size:13.5px;color:#0C5A8A;text-align:right;font-family:inherit}
...
.aibtn{background:#0C5A8A;color:#fff;border:none;border-radius:12px;padding:13px 20px;font-weight:800;cursor:pointer;font-size:14px;margin-top:4px}
.aibtn:disabled{opacity:.6}
.aibox{white-space:pre-wrap;background:#084563;color:#E7ECF6;border-radius:14px;padding:16px 18px;margin-top:12px;font-size:13px;line-height:1.65;font-weight:500;display:none}
...
.b-dark{background:#0C5A8A;color:#fff;box-shadow:0 6px 16px rgba(16,27,48,.2)}
.b-gold{background:#3FB6E8;color:#0C5A8A;box-shadow:0 6px 16px rgba(248,180,8,.3)}
.b-line{background:#fff;border:1px solid #E4E7EC;color:#0C5A8A;box-shadow:0 1px 2px rgba(16,27,48,.04)}
.b-red{background:#FDECEC;color:#C5221F}
table{width:100%;border-collapse:collapse;font-size:13.5px}
th{text-align:left;color:#9097A3;font-size:10.5px;letter-spacing:.7px;text-transform:uppercase;font-weight:700;padding:10px;border-bottom:1px solid #EEF0F4}
td{padding:13px 10px;border-bottom:1px solid #F2F4F7;font-weight:600;color:#0E5180}
.sw{width:18px;height:18px;border-radius:6px;display:inline-block;vertical-align:middle;border:1px solid rgba(0,0,0,.1)}
</style></head><body>
<header><b style="color:#fff;font-weight:900">Maid<span style="color:#3FB6E8">Flow</span></b><a href="/admin">← Tablero</a></header>
<div class="wrap">
...
*{box-sizing:border-box;font-family:Inter,Arial,sans-serif;margin:0}
body{background:#0C5A8A;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}
...
.wordmark{font-size:26px;font-weight:900;color:#0C5A8A;margin-bottom:12px}h1{font-size:19px;color:#0C5A8A;margin-bottom:8px}
p{color:#5A6478;font-size:14px;font-weight:600;line-height:1.6}
a{display:inline-block;margin-top:18px;background:#1F9BD2;color:#fff;text-decoration:none;font-weight:800;padding:13px 24px;border-radius:12px}
</style></head><body><div class="card">
<div class="wordmark">Maid<span style="color:#3FB6E8">Flow</span></div>
...
*{box-sizing:border-box;font-family:Inter,Arial,sans-serif;margin:0}body{background:#F4F6FA;color:#0C5A8A;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:20px}
...
a{display:inline-block;margin-top:18px;background:#0C5A8A;color:#fff;text-decoration:none;font-weight:800;padding:14px 26px;border-radius:12px}
</style></head><body><div class="card">
...
body{margin:0;background:#F4F6FA;color:#0C5A8A}
...
input:focus{border-color:#3FB6E8}
select{width:100%;padding:14px;border:1.5px solid #E6E8EC;border-radius:12px;font-size:16px;font-weight:600;outline:none;margin-bottom:10px;background:#fff;color:#0C5A8A;-webkit-appearance:none}
select:focus{border-color:#3FB6E8}
...
.btn{width:100%;padding:15px;border:none;border-radius:12px;background:#3FB6E8;color:#fff;font-size:16px;font-weight:800;cursor:pointer}
...
.spin{width:46px;height:46px;border:5px solid #E9F6FD;border-top-color:#3FB6E8;border-radius:50%;margin:0 auto 14px;animation:sp 1s linear infinite}
...
.range{background:#0C5A8A;border-radius:14px;padding:16px;text-align:center;margin-bottom:12px}
.range .lbl{color:#3FB6E8;font-size:11px;font-weight:800;letter-spacing:2px}
...
.manual{background:#E9F6FD;border:1.5px solid #3FB6E8;color:#7A5A00;border-radius:12px;padding:12px;font-weight:700;font-size:13px;line-height:1.5;margin:12px 0}
.call{display:block;text-align:center;text-decoration:none;margin-top:12px;padding:15px;border-radius:12px;background:#0C5A8A;color:#fff;font-weight:800;font-size:16px}
...
function landingPage(req) {
  ...
  const L = en ? {
    ...
    s2t: "They leave their phone to see the price", s2x: `<b style="color:#1F9BD2">No name and phone, no price.</b> The engine sizes the home and calculates a cleaning price range — instantly, branded as you.`,
    ...
    trySub: `Esto es lo que verán los dueños en TU página web — con tu logo y <b style="color:#0C5A8A">tu marca</b>. Escriben una dirección de verdad y miran cómo cotiza la limpieza según el tamaño de la casa. El precio sale en <b style="color:#0C5A8A">rango</b>, y para verlo dejan su nombre y teléfono — ese es tu cliente.`,
    ...
    s2t: "Deja su teléfono para ver el precio", s2x: `<b style="color:#1F9BD2">Sin nombre y teléfono, no hay precio.</b> El motor mide la casa y calcula un rango de precio de limpieza — al instante, con tu marca.`,
...
body{background:#fff;color:#0C5A8A}
...
.langpill{position:fixed;top:14px;right:16px;z-index:50;background:#0C5A8A;color:#fff;border-radius:99px;padding:9px 17px;font-weight:800;font-size:13px;text-decoration:none;box-shadow:0 10px 26px rgba(16,27,48,.3)}
...
.hero h1 em{color:#3FB6E8;font-style:normal}
.hero p{color:#5A6478;font-size:clamp(15px,2.5vw,19px);font-weight:600;margin:18px auto 0;max-width:620px;line-height:1.55}
.cta{display:inline-block;margin-top:30px;background:#3FB6E8;color:#0C5A8A;font-weight:800;font-size:17px;padding:17px 36px;border-radius:14px;text-decoration:none;box-shadow:0 14px 34px rgba(248,180,8,.35)}
.cta2{display:inline-block;margin-top:30px;margin-left:12px;color:#0C5A8A;font-weight:700;font-size:15px;padding:17px 24px;text-decoration:none;border:1.5px solid #DDE3EE;border-radius:14px}
...
.dark{background:#0C5A8A;color:#fff}
...
.sec-t em{color:#3FB6E8;font-style:normal}
...
.step .n{font-family:'Barlow Condensed',sans-serif;color:#3FB6E8;font-size:44px;font-weight:800}
...
.phone{width:280px;background:#084563;border:10px solid #0E5180;border-radius:42px;padding:18px 14px 26px;box-shadow:0 36px 90px rgba(0,0,0,.45)}
.notch{width:110px;height:22px;background:#0E5180;border-radius:0 0 14px 14px;margin:-18px auto 14px}
.papp{background:#F4F6FA;border-radius:18px;padding:12px;color:#0C5A8A}
.phead{font-weight:800;font-size:15px;display:flex;align-items:center;gap:8px;margin-bottom:10px}
.pbadge{background:#3FB6E8;color:#fff;border-radius:99px;font-size:11px;font-weight:800;padding:3px 10px;margin-left:auto}
.plead{background:#fff;border:2px solid #3FB6E8;border-radius:14px;padding:12px;font-size:13px;line-height:1.5}
.pnew{background:#3FB6E8;color:#fff;border-radius:99px;font-size:10px;font-weight:800;padding:2px 8px}
.gold{color:#1F9BD2;font-weight:800}
...
.qfill{height:100%;width:20%;background:#3FB6E8;border-radius:99px;transition:width .3s ease}
.qstep{display:none}
.qstep.on{display:block}
.qq{font-weight:800;font-size:19px;text-align:center;margin-bottom:18px}
.opts{display:grid;grid-template-columns:1fr 1fr;gap:12px}
.opt{background:#fff;border:1.5px solid #DDE3EE;border-radius:14px;padding:18px 12px;font-weight:700;font-size:15px;color:#0C5A8A;cursor:pointer;box-shadow:0 6px 18px rgba(16,27,48,.05)}
.opt:hover{border-color:#3FB6E8;background:#FFFBEF}
...
input{width:100%;padding:15px;border-radius:12px;border:1.5px solid #DDE3EE;background:#fff;color:#0C5A8A;font-size:16px;font-weight:600;margin-bottom:10px;outline:none}
input:focus{border-color:#3FB6E8}
button{width:100%;padding:17px;border:none;border-radius:12px;background:#3FB6E8;color:#0C5A8A;font-size:17px;font-weight:800;cursor:pointer;box-shadow:0 12px 30px rgba(248,180,8,.3)}
...
<div class="wrap">
<nav><span class="lg" style="font-family:'Barlow Condensed',sans-serif;font-weight:800;font-size:40px;letter-spacing:.5px;color:#0C5A8A">Maid<span style="color:#3FB6E8">Flow</span></span></nav>
<div class="hero">
  <h1>${L.h1}</h1>
  <p>${L.sub}</p>
...
.ribbon{background:#3FB6E8;color:#0C5A8A;text-align:center;font-weight:800;font-size:12.5px;padding:9px 14px;letter-spacing:.2px}
...
.backalto{position:fixed;bottom:18px;left:50%;transform:translateX(-50%);z-index:50;background:#0C5A8A;color:#fff;text-decoration:none;font-weight:800;font-size:14px;padding:13px 22px;border-radius:99px;box-shadow:0 14px 36px rgba(16,27,48,.5);display:flex;align-items:center;gap:8px;white-space:nowrap}
.backalto span{color:#3FB6E8}
...
${pPhone ? `<a href="tel:+1${pPhone}">📞 Llámanos / Call us</a>` : ""}
  ...
  if (!published && !preview) {
    ...
    const cColor = /^#[0-9a-fA-F]{6}$/.test(String(c.data?.site?.color || "")) ? c.data.site.color : "#0C5A8A";
    return res.send(`<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
...
.card{background:#fff;color:#0C5A8A;border-radius:26px;padding:40px 30px;max-width:440px;width:100%;text-align:center;box-shadow:0 30px 80px rgba(0,0,0,.4)}
...
.done b{color:#34A853}.wip b{color:#1F9BD2}
...
*{box-sizing:border-box;font-family:Inter,Arial,sans-serif;margin:0}
body{background:#0C5A8A;color:#fff;padding:34px 20px 60px}
h1{text-align:center;font-size:clamp(24px,4.5vw,36px);font-weight:800}
h1 em{color:#3FB6E8;font-style:normal}
...
.colorbar button{background:#3FB6E8;color:#0C5A8A;border:none;border-radius:10px;padding:10px 18px;font-weight:800;cursor:pointer}
.grid{display:flex;gap:34px;justify-content:center;flex-wrap:wrap}
.card{text-align:center}
.phone{background:#084563;border:9px solid #0E5180;border-radius:40px;padding:10px;box-shadow:0 26px 70px rgba(0,0,0,.5)}
...
.nm span{color:#3FB6E8}
.ds{color:#9DA8C4;font-weight:600;font-size:13px;margin-top:4px}
.open{display:inline-block;margin-top:12px;background:#fff;color:#0C5A8A;text-decoration:none;font-weight:800;font-size:13px;padding:10px 20px;border-radius:99px}
</style></head><body>
...
<div class="colorbar">
  ...
  <button onclick="reset()" style="background:#0E5180;color:#fff">Colores originales</button>
</div>
<div class="grid">
${T.map(([n, nm, ds]) => `
  <div class="card">
...
.card.gold{background:linear-gradient(155deg,#16243f,#0d1729);border:none}.card.gold .v{color:#3FB6E8}.card.gold .l{color:#9DA8C4}
...
.tform select:focus,.tform input:focus{border-color:#3FB6E8;box-shadow:0 0 0 3px rgba(248,180,8,.18)}
.tform button{background:#3FB6E8;color:#0C5A8A;border:none;border-radius:11px;padding:11px 20px;font-weight:800;cursor:pointer;white-space:nowrap}
...
.tbtn{border:1px solid #E4E7EC;background:#fff;border-radius:9px;padding:6px 11px;font-weight:700;font-size:12px;cursor:pointer;text-decoration:none;color:#0C5A8A}
.tbtn.go{background:#0C5A8A;color:#fff;border:none}.tbtn.del{color:#C5221F;border-color:#F3B4B0}
...
td a{color:#B07A00;font-weight:700;text-decoration:none}
.edit{background:#3FB6E8;color:#0C5A8A !important;border-radius:9px;padding:6px 12px;font-weight:800;font-size:12.5px}
...
.qchip:hover{border-color:#3FB6E8;background:#FFFBEF}
...
.guide summary::before{content:"▸ ";color:#1F9BD2}
...
*{box-sizing:border-box;font-family:Inter,Arial,sans-serif;margin:0}body{background:#F4F6FA;color:#0C5A8A}
header{background:#0C5A8A;color:#fff;padding:14px 22px;display:flex;align-items:center;gap:12px}
header img{height:32px;background:#fff;border-radius:8px;padding:4px 6px}header b em{color:#3FB6E8;font-style:normal}
...
.row a{background:#3FB6E8;color:#0C5A8A;text-decoration:none;font-weight:800;border-radius:10px;padding:9px 16px;font-size:13px}
...
*{box-sizing:border-box;font-family:Inter,Arial,sans-serif;margin:0;-webkit-tap-highlight-color:transparent}
:root{--navy:#0C5A8A;--navy2:#084563;--gold:#3FB6E8;--mut:#9DA8C4;--line:rgba(255,255,255,.1)}
...
.sb-brand b{color:#0C5A8A;font-weight:800;font-size:15px}.sb-brand b em{color:#1F9BD2;font-style:normal}
...
.nav-it.on{color:#0C5A8A;background:rgba(248,180,8,.13);border-left-color:var(--gold)}
.nav-it.on .no{color:#1F9BD2}
...
.fcard{background:#fff;color:#084563;border-radius:24px;padding:24px 26px;max-width:640px;width:100%;box-shadow:0 30px 80px rgba(0,0,0,.45);margin-top:24px}
label{display:block;font-weight:600;font-size:13px;margin:16px 0 6px;color:#475067}
label:first-child{margin-top:0}
input,textarea,select{width:100%;padding:13px 15px;border-radius:13px;border:1px solid #E4E7EC;font-size:15px;font-weight:500;outline:none;font-family:inherit;color:#084563;background:#fff;transition:border-color .15s,box-shadow .15s}
...
.btn-dark{background:#0C5A8A;color:#fff;border:none;border-radius:11px;padding:12px 18px;font-weight:800;cursor:pointer}
...
.tphone{background:#084563;border:8px solid #0E5180;border-radius:34px;padding:7px;box-shadow:0 22px 60px rgba(0,0,0,.5)}
...
.nb-btn.next{background:var(--gold);color:#0C5A8A;border:none;box-shadow:0 8px 20px rgba(248,180,8,.3)}
.nb-btn:disabled{opacity:.35;cursor:default}
.save{width:100%;padding:16px;border:none;border-radius:14px;background:var(--gold);color:#0C5A8A;font-size:16px;font-weight:800;cursor:pointer;box-shadow:0 10px 26px rgba(248,180,8,.35);transition:transform .12s,filter .15s;margin-top:6px}
...
.rev li b{color:#475067;font-weight:600}.rev li span{font-weight:700;color:#084563;text-align:right}
...
.chip:has(input:checked){border-color:var(--gold);background:#FFFBEF;color:#0C5A8A}
.chip:has(input:checked)::before{content:"✓";color:#1F9BD2;font-weight:900}
...
<main>
  <div class="mtop"><b style="color:#fff;font-weight:800;font-size:15px">Maid<span style="color:#3FB6E8">Flow</span></b><div><div class="mstep" id="mstep">Paso 1 de 7</div><div class="mtitle" id="mtitle">Bienvenida</div></div></div>
  <div class="stage">
    ...
    <section class="slide">
      <div class="s-in top">
        ...
        <div class="fcard">
          ...
          <p style="text-align:center;color:#475067;font-weight:600;font-size:14px;margin:8px 0 18px;line-height:1.6">Revisa que todo esté bien. Al enviar, nuestro equipo de diseño arma tu página a mano y te la entregamos lista en <b style="color:#084563">10–14 días</b>.</p>
          <ul class="rev">
          ...
          <div id="staff" style="display:${st.template || st.about ? "block" : "none"};margin-top:14px;text-align:center">
            ...
            <button onclick="publish()" id="pub" class="btn-dark" style="background:${st.published ? "#1E7B3C" : "#0C5A8A"}">${st.published ? "✓ Publicada — clic para ocultar" : "🚀 Publicar página al cliente"}</button>
          </div>
        </div>
      </div>
    </section>
  ...
  fetch('/api/onboarding/publish?key=${K}',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({slug:${JSON.stringify(c.slug)},publish:!PUBLISHED})})
    .then(function(r){return r.json();}).then(function(j){
      ...
      if(j&&j.ok){PUBLISHED=j.published;
        ...
        pub.style.background=PUBLISHED?'#1E7B3C':'#0C5A8A';
      }
...
.iphone{position:relative;background:#084563;border:9px solid #0E5180;border-radius:44px;padding:10px;box-shadow:0 30px 80px rgba(0,0,0,.55)}
.inotch{position:absolute;top:10px;left:50%;transform:translateX(-50%);width:100px;height:20px;background:#0E5180;border-radius:0 0 12px 12px;z-index:2}
...
<div class="layout">
<aside id="sb"><div class="sb-brand"><b style="color:#0C5A8A;font-weight:900;font-size:22px">Maid<span style="color:#3FB6E8">Flow</span></b></div><div class="sb-label">MAID FLOW</div><nav id="nav"></nav><div class="sb-foot">Presentación del rol</div></aside>
<div class="scrim" id="scrim" onclick="sb(false)"></div>
<main>
<div class="mtop"><button onclick="sb(true)">☰ Menú</button><b style="font-weight:800">Maid<span style="color:#3FB6E8">Flow</span></b><span style="width:64px"></span></div>
<div class="stage" id="stage">

<section class="slide" data-t="El rol">
  <div class="s-bg" style="background:linear-gradient(160deg,#062B22,#0C5A8A)"></div><div class="s-veil"></div>
  <div class="s-in">
...
<style>body{font-family:Arial;max-width:560px;margin:40px auto;padding:0 16px;color:#0C5A8A}h2{margin-bottom:6px}
.link{background:#E9F6FD;border:2px solid #3FB6E8;border-radius:12px;padding:13px;word-break:break-all;font-size:14px;margin:10px 0;display:flex;gap:10px;align-items:center}
.link button{margin-left:auto;background:#3FB6E8;color:#0C5A8A;border:none;border-radius:8px;padding:8px 14px;font-weight:800;cursor:pointer;flex-shrink:0}
...
${paid
  ...
  : `<p style="background:#E9F6FD;border:1.5px solid #3FB6E8;color:#7A5A00;border-radius:12px;padding:10px 14px;font-weight:700">⏳ El link de acceso se ACTIVA solo cuando Stripe confirme su pago (≈1 min después de pagar). Si pagó por otro medio, el admin la activa desde su tablero.</p>`}
...
<div class="link"><span><b>🎨 Onboarding de su página</b></span><a href="/onboarding?key=${K}&slug=${c.slug}" style="margin-left:auto;background:#3FB6E8;color:#0C5A8A;border-radius:8px;padding:8px 14px;font-weight:800;text-decoration:none">Abrir →</a></div>
...
body{max-width:680px;margin:0 auto;padding:34px 20px 72px;color:#084563;line-height:1.55;letter-spacing:-0.011em}
::selection{background:rgba(248,180,8,.35)}
h1{font-size:26px;font-weight:700;letter-spacing:-0.025em;margin-bottom:18px}
h1 span{color:#1F9BD2}
...
.alta input:focus{border-color:#3FB6E8;box-shadow:0 0 0 4px rgba(248,180,8,.18)}
.alta button{background:#3FB6E8;color:#0C5A8A;border:none;border-radius:13px;padding:13px 24px;font-weight:700;cursor:pointer;font-size:14.5px;box-shadow:0 6px 16px rgba(248,180,8,.3);transition:transform .12s,filter .15s}
.alta button:hover{filter:brightness(1.03)}.alta button:active{transform:scale(.97)}
ol{padding-left:22px;margin-top:4px}ol li{margin-bottom:10px;font-weight:500;color:#0E5180}
ul{padding-left:22px}ul li{margin-bottom:7px;font-weight:500;color:#0E5180}
small{color:#9097A3}
.sc{background:#fff;border:1px solid rgba(16,27,48,.05);border-left:3px solid #3FB6E8;border-radius:16px;padding:16px 18px;margin:10px 0;box-shadow:0 1px 2px rgba(16,27,48,.04),0 8px 22px rgba(16,27,48,.045)}
...
.sc p{font-size:15px;font-style:italic;color:#0E5180;line-height:1.6}
...
.link button{background:#3FB6E8;color:#0C5A8A;border:none;border-radius:11px;padding:9px 15px;font-weight:700;cursor:pointer;flex-shrink:0;font-size:13px;transition:filter .15s}
...
.ob b{font-size:14px;font-weight:700;color:#084563}
...
.navbtn{display:inline-flex;align-items:center;gap:8px;background:#fff;border:1px solid rgba(16,27,48,.06);border-radius:14px;padding:13px 20px;font-weight:700;font-size:14.5px;color:#084563;text-decoration:none;box-shadow:0 1px 2px rgba(16,27,48,.04),0 8px 22px rgba(16,27,48,.05);transition:transform .15s,box-shadow .15s}
.navbtn:hover{transform:translateY(-1px);box-shadow:0 2px 4px rgba(16,27,48,.06),0 14px 30px rgba(16,27,48,.09)}
.navbtn.primary{background:#3FB6E8;border:none;box-shadow:0 6px 18px rgba(248,180,8,.35)}
...
.card.gold .v{color:#3FB6E8}.card.gold .l{color:#9DA8C4}
...
.mform input:focus{border-color:#3FB6E8;box-shadow:0 0 0 4px rgba(248,180,8,.18)}
.mform button{background:#0C5A8A;color:#fff;border:none;border-radius:12px;padding:12px 20px;font-weight:700;cursor:pointer;font-size:14px}
...
.mbtn.show{background:#E7F7ED;color:#10803C}.mbtn.no{background:#FDECEC;color:#C5221F}.mbtn.win{background:#3FB6E8;color:#0C5A8A}
...
.mnote{flex-basis:100%;font-family:inherit;margin-top:4px;padding:9px 12px;border-radius:10px;border:1px solid #E4E7EC;font-size:13px;font-weight:500;color:#0E5180;outline:none;transition:border-color .2s,box-shadow .15s}
.mnote::placeholder{color:#B6BCC8;font-weight:500}
.mnote:focus{border-color:#3FB6E8;box-shadow:0 0 0 3px rgba(248,180,8,.16)}
...
<div class="cols">
  <div class="col">
    ...
    <h2>${L.linksT}</h2>
    ${stripeLink
      ? `<div class="link"><span><b>${L.payT}</b><br><small>${esc(stripeLink)}</small></span><a href="${stripeLink}" target="_blank" rel="noreferrer" style="background:#0C5A8A;color:#fff;border-radius:11px;padding:9px 15px;font-weight:700;text-decoration:none;flex-shrink:0;font-size:13px">${L.open}</a><button onclick="cp(this,'${stripeLink}')">${L.copy}</button></div>`
...
body{font-family:Arial;max-width:640px;margin:30px auto;padding:0 18px;color:#0C5A8A;line-height:1.55}
...
.link{background:#E9F6FD;border:2px solid #3FB6E8;border-radius:12px;padding:12px;word-break:break-all;font-size:14px;margin:8px 0;display:flex;gap:10px;align-items:center}
.link button{margin-left:auto;background:#3FB6E8;color:#0C5A8A;border:none;border-radius:8px;padding:8px 14px;font-weight:800;cursor:pointer;flex-shrink:0}
.link small{color:#67718A}
ol li{margin-bottom:10px}small{color:#67718A}
</style></head><body>
<h1>🔒 Cierre · Maid<span style="color:#1F9BD2">Flow</span></h1>
...
.iphone{position:relative;background:#084563;border:10px solid #0E5180;border-radius:48px;padding:11px;box-shadow:0 30px 80px rgba(0,0,0,.55)}
.inotch{position:absolute;top:11px;left:50%;transform:translateX(-50%);width:110px;height:22px;background:#0E5180;border-radius:0 0 13px 13px;z-index:2}
...
.pcard{background:#fff;color:#0C5A8A;border-radius:26px;padding:34px 32px;text-align:center;box-shadow:0 34px 90px rgba(248,180,8,.18),0 30px 70px rgba(0,0,0,.45);width:min(340px,100%)}
.pcard .pk{color:#1F9BD2;font-weight:800;font-size:11px;letter-spacing:2.5px}
...
.bub.me{background:#0C5A8A;color:#fff;margin-left:auto;border-bottom-right-radius:5px}
...
.ch-in input:focus{border-color:#3FB6E8}
.ch-in button{background:#3FB6E8;color:#0C5A8A;border:none;border-radius:11px;padding:0 18px;font-weight:800;font-size:17px;cursor:pointer}
...
.bbar .next{background:var(--gold);color:var(--navy);border:none}
.bbar .ct{font-family:'Fraunces',Georgia,serif;font-size:15px;color:var(--mut)}
.langpill{position:fixed;top:16px;right:18px;z-index:45;background:rgba(255,255,255,.95);color:#0C5A8A;border-radius:99px;padding:9px 18px;font-weight:800;font-size:13px;text-decoration:none;box-shadow:0 10px 28px rgba(0,0,0,.35)}
...
#ckit{display:none;position:fixed;right:18px;bottom:70px;z-index:80;background:#fff;color:#0C5A8A;border-radius:16px;padding:14px 16px;box-shadow:0 24px 60px rgba(0,0,0,.5);width:280px}
...
#ckit button{background:#3FB6E8;color:#0C5A8A;border:none;border-radius:8px;padding:6px 12px;font-weight:800;font-size:12px;cursor:pointer}
...
<div class="layout">
<aside id="sb">
  <div class="sb-brand"><b style="color:#0C5A8A;font-weight:900;font-size:24px">Maid<span style="color:#3FB6E8">Flow</span></b></div>
...
<section class="slide" data-t="${L.t3}">
  ...
  <div class="s-in" style="max-width:1150px">
    ...
    <p class="body" style="margin-top:28px;font-size:clamp(17px,2.3vw,23px);max-width:940px"><b style="color:#3FB6E8">${L.c3}</b></p>
  </div>
</section>
...
<style>
  body{margin:0;font-family:-apple-system,'Segoe UI',Roboto,sans-serif;background:#F4F5F7;color:#0C5A8A}
  .page{max-width:560px;margin:0 auto;background:#fff;min-height:100vh}
  .hd{background:#0C5A8A;color:#fff;padding:22px 24px}
  .hd .biz{font-size:22px;font-weight:800;letter-spacing:.02em}
  .hd .sub{color:#9DA8C4;font-size:13px;margin-top:2px}
  .tag{display:inline-block;background:#3FB6E8;color:#fff;font-size:12px;font-weight:800;border-radius:99px;padding:3px 12px;margin-top:10px;letter-spacing:.06em}
  ...
  .due .amt{color:${d.paid ? "#1E9E5A" : "#3FB6E8"}}
  .pay{background:#E9F6FD;border-radius:12px;padding:14px 16px;font-size:15px}
  .pay b{display:block;font-size:11px;letter-spacing:.1em;color:#3FB6E8;margin-bottom:6px}
  .btn{display:block;width:calc(100% - 48px);margin:18px 24px;background:#3FB6E8;color:#fff;border:none;border-radius:12px;padding:15px;font-size:16px;font-weight:800;cursor:pointer}
...
<div style="display:flex;gap:24px;margin-top:34px">
  <div style="flex:2;border-top:1.5px solid #0C5A8A;padding-top:5px">${L.sig}</div>
  <div style="flex:1;border-top:1.5px solid #0C5A8A;padding-top:5px">${L.sigDate}</div>
</div></div>` : ""}
