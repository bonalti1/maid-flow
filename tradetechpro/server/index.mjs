 ...
 * the Maps key stays server-side. Street View when Google has coverage, else a
 * top-down satellite frame. 404 (UI hides it) when no key / no coordinates. */
app.get("/api/housephoto", async (req, res) => {
...
.mflogo span{color:#6B3FA0}
h1{font-size:18px;color:#0C2A43;margin-bottom:4px}
p{color:#5A7488;font-size:13px;font-weight:600;margin-bottom:18px}
input{width:100%;padding:14px;border-radius:12px;border:1.5px solid #E3E9F4;font-size:16px;font-weight:600;outline:none;text-align:center}
input:focus{border-color:#6B3FA0}
button{width:100%;margin-top:10px;padding:14px;border:none;border-radius:12px;background:#6B3FA0;color:#fff;font-size:16px;font-weight:800;cursor:pointer}
.err{color:#D93025;font-size:13px;font-weight:700;margin-top:10px}
</style></head><body><form class="card" method="get" action="${action}">
<div class="mflogo">pauleza</div>
<h1>${title}</h1>
...
<style>body{font-family:Inter,Arial,sans-serif;max-width:560px;margin:40px auto;padding:0 16px;color:#6B3FA0}h2{margin-bottom:6px}
.link{background:#F7EFD8;border:2px solid #9355C1;border-radius:12px;padding:14px;word-break:break-all;font-size:14px;margin:14px 0}
a{color:#9355C1;font-weight:800}</style></head><body>
...
<style>body{font-family:Inter,Arial,sans-serif;max-width:560px;margin:40px auto;padding:0 16px;color:#6B3FA0}
.link{background:#F7EFD8;border:2px solid #9355C1;border-radius:12px;padding:14px;word-break:break-all;font-size:14px;margin:14px 0}
a{color:#9355C1;font-weight:800}</style></head><body>
...
app.get("/api/admin/backup", async (req, res) => {
  ...
  try {
    ...
    res.setHeader("Content-Disposition", `attachment; filename="pauleza-respaldo-${stamp}.json"`);
    res.send(JSON.stringify({ product: "pauleza", exportedAt: new Date().toISOString(), ...data }, null, 2));
...
app.post("/api/admin/restore", express.json({ limit: "25mb" }), async (req, res) => {
  ...
  if (!dump || typeof dump !== "object" || (!Array.isArray(dump.contractors) && !Array.isArray(dump.clients))) {
    return res.status(400).json({ error: "el archivo no parece un respaldo de Pauleza (falta la lista de clientas)" });
  }
  try {
...
<style>body{font-family:Inter,Arial,sans-serif;max-width:560px;margin:40px auto;padding:0 16px;color:#2A2352}
.link{background:#F1ECFA;border:2px solid #76C7C0;border-radius:12px;padding:14px;word-break:break-all;font-size:14px;margin:14px 0}
a{color:#6B3FA0;font-weight:800}</style></head><body>
...
header b{font-size:16px;font-weight:700;letter-spacing:-0.02em}header b em{color:#76C7C0;font-style:normal}
...
.chart .num{font-size:11px;font-weight:800;color:#2A2352}
...
td{padding:14px 10px;border-bottom:1px solid #F2F4F7;font-weight:600;color:#7D5AB5;vertical-align:middle}
...
.newform input:focus{border-color:#76C7C0;box-shadow:0 0 0 4px rgba(248,180,8,.18)}
.newform button{background:#76C7C0;color:#6B3FA0;border:none;border-radius:13px;padding:13px 24px;font-weight:700;cursor:pointer;font-size:14.5px;transition:transform .12s,filter .15s;box-shadow:0 6px 16px rgba(248,180,8,.3)}
...
.seg.on{background:#fff;color:#6B3FA0;box-shadow:0 1px 3px rgba(16,27,48,.12)}
.segcustom{display:inline-flex;gap:7px;align-items:center}
.segcustom input{font-family:inherit;padding:8px 10px;border-radius:10px;border:1px solid #E4E7EC;font-size:13px;font-weight:600;color:#7D5AB5;outline:none}
.segcustom input:focus{border-color:#76C7C0;box-shadow:0 0 0 3px rgba(248,180,8,.18)}
.segcustom button{background:#6B3FA0;color:#fff;border:none;border-radius:10px;padding:9px 16px;font-weight:700;font-size:13px;cursor:pointer}
.segcustom button.on{background:#76C7C0;color:#6B3FA0}
...
.sub.gold .v{color:#76C7C0}.sub.gold .l{color:#9DA8C4}
...
.lprev{width:134px;height:86px;border-radius:10px;overflow:hidden;border:1px solid #E4E7EC;flex-shrink:0;background:#2A2352;box-shadow:0 4px 12px rgba(16,27,48,.08)}
...
<header><b style="color:#fff;font-weight:900">pauleza</b><b>· Admin</b><span class="tag"><a href="/admin/economics" style="color:#76C7C0">🧮 Calculadora</a> · <a href="/cs" style="color:#9DA8C4">🎧 Servicio</a> · <a href="/admin?logout" style="color:#9DA8C4">salir</a></span></header>
<div class="wrap">
...
<div class="panel">
  ...
  ${[
    ...
    ${links.map(([name, url]) => {
      ...
      <span class="lbtns">
        <button onclick="cpy(this,'${url}')" style="background:#76C7C0;color:#6B3FA0;border:none;border-radius:8px;padding:7px 12px;font-weight:800;cursor:pointer;font-size:12px">Copiar</button>
        <a href="${url}" target="_blank" style="background:#6B3FA0;color:#fff;border-radius:8px;padding:7px 12px;font-weight:800;text-decoration:none;font-size:12px">Abrir</a>
      </span>
    </div>`; }).join("")}
  `).join("")}
  ...
  <div class="lrow" style="align-items:center">
    ...
    <span class="lbtns"><label style="background:#6B3FA0;color:#fff;border-radius:8px;padding:7px 12px;font-weight:800;cursor:pointer;font-size:12px">Elegir archivo<input type="file" accept="application/json" style="display:none" onchange="restoreFile(this)"></label></span>
  </div>
...
<div class="panel">
  ...
  ${list.map((c) => {
    ...
    return `<tr data-name="${esc(c.name).toLowerCase()} ${c.slug}">
      <td><a href="/admin/c/${c.slug}" style="color:#6B3FA0;font-weight:800">${esc(c.name)}</a>${isB ? ' <span class="pill gold">interno</span>' : ""}</td>
      <td>${isPaused
...
.appheader b{font-size:16px;font-weight:700;letter-spacing:-0.02em}.appheader b em{color:#76C7C0;font-style:normal}
...
.card.gold .v{color:#76C7C0}.card.gold .l{color:#9DA8C4}.card.gold .s{color:#9DA8C4}
...
.fld input[type=range]{flex:1;accent-color:#76C7C0}
.fld .val{min-width:78px;display:flex;align-items:center;background:#F4F6FA;border:1px solid #E4E7EC;border-radius:9px;padding:6px 9px;font-weight:800;font-size:13.5px;color:#6B3FA0}
.fld .val .pre{color:#9097A3;font-weight:700;margin-right:2px}
.fld .val input{width:100%;border:none;background:none;outline:none;font-weight:800;font-size:13.5px;color:#6B3FA0;text-align:right;font-family:inherit}
...
.aibtn{background:#6B3FA0;color:#fff;border:none;border-radius:12px;padding:13px 20px;font-weight:800;cursor:pointer;font-size:14px;margin-top:4px}
.aibtn:disabled{opacity:.6}
.aibox{white-space:pre-wrap;background:#2A2352;color:#E7ECF6;border-radius:14px;padding:16px 18px;margin-top:12px;font-size:13px;line-height:1.65;font-weight:500;display:none}
...
<div class="appheader">
  <b>pauleza · ${tr("Centro de mando", "Command center")}</b>
...
.b-dark{background:#6B3FA0;color:#fff;box-shadow:0 6px 16px rgba(16,27,48,.2)}
.b-gold{background:#76C7C0;color:#6B3FA0;box-shadow:0 6px 16px rgba(248,180,8,.3)}
.b-line{background:#fff;border:1px solid #E4E7EC;color:#6B3FA0;box-shadow:0 1px 2px rgba(16,27,48,.04)}
.b-red{background:#FDECEC;color:#C5221F}
table{width:100%;border-collapse:collapse;font-size:13.5px}
th{text-align:left;color:#9097A3;font-size:10.5px;letter-spacing:.7px;text-transform:uppercase;font-weight:700;padding:10px;border-bottom:1px solid #EEF0F4}
td{padding:13px 10px;border-bottom:1px solid #F2F4F7;font-weight:600;color:#7D5AB5}
.sw{width:18px;height:18px;border-radius:6px;display:inline-block;vertical-align:middle;border:1px solid rgba(0,0,0,.1)}
</style></head><body>
<header><b style="color:#fff;font-weight:900">pauleza</b><a href="/admin">← Tablero</a></header>
<div class="wrap">
...
<title>Pauleza</title><style>
*{box-sizing:border-box;font-family:Inter,Arial,sans-serif;margin:0}
body{background:#6B3FA0;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}
...
.wordmark{font-size:26px;font-weight:900;color:#6B3FA0;margin-bottom:12px}h1{font-size:19px;color:#6B3FA0;margin-bottom:8px}
p{color:#5A6478;font-size:14px;font-weight:600;line-height:1.6}
a{display:inline-block;margin-top:18px;background:#9355C1;color:#fff;text-decoration:none;font-weight:800;padding:13px 24px;border-radius:12px}
</style></head><body><div class="card">
<div class="wordmark">pauleza</div>
...
function sanitizeRates(r) {
  ...
  for (const grp of ["CONDITION", "PETS", "FURNISHED", "FREQ_DISCOUNT", "ADDON"]) {
    ...
    // ADDON: keep the cleaner's own custom extras (well-formed custom:* keys)
    if (grp === "ADDON") {
      for (const k of Object.keys(r.ADDON)) {
        if (k in RATE_DEFAULTS.ADDON || !/^custom:[a-z0-9]{1,32}$/.test(k)) continue;
        const v = posNum(r.ADDON[k], 1e4); if (v !== undefined) g[k] = v;
      }
    }
    if (Object.keys(g).length) out[grp] = g;
  }
  // Display names for the custom extras (strings, not prices).
  if (r.ADDON_LABELS && typeof r.ADDON_LABELS === "object") {
    const L = {};
    for (const k of Object.keys(r.ADDON_LABELS)) {
      if (!/^custom:[a-z0-9]{1,32}$/.test(k)) continue;
      const name = String(r.ADDON_LABELS[k] || "").replace(/[<>]/g, "").slice(0, 40).trim();
      if (name) L[k] = name;
    }
    if (Object.keys(L).length) out.ADDON_LABELS = L;
  }
...
function forwardLead(c, lead) {
  ...
  fetch(hook, {
    ...
    body: JSON.stringify({ source: "pauleza", contractor: c.slug, ...lead }),
...
app.get("/q/:id", async (req, res) => {
  ...
  const biz = p.biz || c.name || "Pauleza";
...
app.get("/api/leads.csv", async (req, res) => {
  ...
  res.setHeader("Content-Disposition", `attachment; filename="pauleza-leads-${new Date().toISOString().slice(0, 10)}.csv"`);
...
*{box-sizing:border-box;font-family:Inter,Arial,sans-serif;margin:0}body{background:#F4F6FA;color:#6B3FA0;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:20px}
...
a{display:inline-block;margin-top:18px;background:#6B3FA0;color:#fff;text-decoration:none;font-weight:800;padding:14px 26px;border-radius:12px}
</style></head><body><div class="card">
...
body{margin:0;background:#F4F6FA;color:#6B3FA0}
...
input:focus{border-color:#76C7C0}
select{width:100%;padding:14px;border:1.5px solid #E6E8EC;border-radius:12px;font-size:16px;font-weight:600;outline:none;margin-bottom:10px;background:#fff;color:#6B3FA0;-webkit-appearance:none}
select:focus{border-color:#76C7C0}
...
.btn{width:100%;padding:15px;border:none;border-radius:12px;background:#76C7C0;color:#fff;font-size:16px;font-weight:800;cursor:pointer}
...
.sug button:active{background:#F1ECFA}
...
.spin{width:46px;height:46px;border:5px solid #F1ECFA;border-top-color:#76C7C0;border-radius:50%;margin:0 auto 14px;animation:sp 1s linear infinite}
...
.range{background:#6B3FA0;border-radius:14px;padding:16px;text-align:center;margin-bottom:12px}
.range .lbl{color:#76C7C0;font-size:11px;font-weight:800;letter-spacing:2px}
...
.manual{background:#F1ECFA;border:1.5px solid #76C7C0;color:#7A5A00;border-radius:12px;padding:12px;font-weight:700;font-size:13px;line-height:1.5;margin:12px 0}
.call{display:block;text-align:center;text-decoration:none;margin-top:12px;padding:15px;border-radius:12px;background:#6B3FA0;color:#fff;font-weight:800;font-size:16px}
...
var L=${JSON.stringify({ m1: L.m1, m2: L.m2, m3: L.m3, range: L.range, rangeSub: L.rangeSub, sent: L.sent, callTxt: L.call(prof.biz || c.name), nores: L.nores, noresSub: L.noresSub(prof.biz || c.name), sqftQ: L.sqftQ, sqftPh: L.sqftPh, sqftBtn: L.sqftBtn, callBtn: L.callBtn, err: L.err,
  ...
  manual: c.slug === "alto-demo" ? (es
    ? "👆 Este es el imán de clientes. En tu app Pauleza armas la cotización completa con tus precios y la mandas por WhatsApp — para captar y cerrar con confianza."
    : "👆 This is the lead magnet. In your Pauleza app you build the full quote with your prices and send it on WhatsApp — to capture and close with confidence.") : null }).replace(/</g, "\\u003c")};
...
function landingPage(req) {
  ...
  const L = en ? {
    ...
    title: "Pauleza — Your website finds you cleaning jobs by itself",
    ...
    ogTitle: "Pauleza — Your website finds you cleaning jobs by itself",
    ...
    s2t: "They leave their phone to see the price", s2x: `<b style="color:#9355C1">No name and phone, no price.</b> The engine sizes the home and calculates a cleaning price range — instantly, branded as you.`,
    ...
    plans: [
      { key: "pro", name: "PRO", amt: 49, desc: "The app in your pocket", feats: ["The Pauleza app: quotes in 60 seconds", "Your own rates and minimums", "Branded quotes, sent by WhatsApp", "Your customer and lead list"] },
    ...
    foot: `Pauleza · Made in Texas 🤠`,
  } : {
    lang: "es", langBtn: "🇺🇸 English", langHref: "/?lang=en",
    title: "Pauleza — Tu página web te consigue trabajos de limpieza sola",
    ...
    ogTitle: "Pauleza — Tu página web te consigue trabajos de limpieza sola",
    ...
    trySub: `Esto es lo que verán los dueños en TU página web — con tu logo y <b style="color:#6B3FA0">tu marca</b>. Escriben una dirección de verdad y miran cómo cotiza la limpieza según el tamaño de la casa. El precio sale en <b style="color:#6B3FA0">rango</b>, y para verlo dejan su nombre y teléfono — ese es tu cliente.`,
    ...
    s2t: "Deja su teléfono para ver el precio", s2x: `<b style="color:#9355C1">Sin nombre y teléfono, no hay precio.</b> El motor mide la casa y calcula un rango de precio de limpieza — al instante, con tu marca.`,
    ...
    plans: [
      { key: "pro", name: "PRO", amt: 49, desc: "La app en tu bolsillo", feats: ["La app Pauleza: cotizas en 60 segundos", "Tus propias tarifas y mínimos", "Cotizaciones con tu marca, por WhatsApp", "Tu lista de clientes y leads"] },
...
body{background:#fff;color:#6B3FA0}
...
.langpill{position:fixed;top:14px;right:16px;z-index:50;background:#6B3FA0;color:#fff;border-radius:99px;padding:9px 17px;font-weight:800;font-size:13px;text-decoration:none;box-shadow:0 10px 26px rgba(16,27,48,.3)}
...
.hero h1 em{color:#76C7C0;font-style:normal}
.hero p{color:#5A6478;font-size:clamp(15px,2.5vw,19px);font-weight:600;margin:18px auto 0;max-width:620px;line-height:1.55}
.cta{display:inline-block;margin-top:30px;background:#76C7C0;color:#6B3FA0;font-weight:800;font-size:17px;padding:17px 36px;border-radius:14px;text-decoration:none;box-shadow:0 14px 34px rgba(248,180,8,.35)}
.cta2{display:inline-block;margin-top:30px;margin-left:12px;color:#6B3FA0;font-weight:700;font-size:15px;padding:17px 24px;text-decoration:none;border:1.5px solid #DDE3EE;border-radius:14px}
...
.dark{background:#6B3FA0;color:#fff}
...
.sec-t em{color:#76C7C0;font-style:normal}
...
.step .n{font-family:'Barlow Condensed',sans-serif;color:#76C7C0;font-size:44px;font-weight:800}
...
.phone{width:280px;background:#2A2352;border:10px solid #7D5AB5;border-radius:42px;padding:18px 14px 26px;box-shadow:0 36px 90px rgba(0,0,0,.45)}
.notch{width:110px;height:22px;background:#7D5AB5;border-radius:0 0 14px 14px;margin:-18px auto 14px}
.papp{background:#F4F6FA;border-radius:18px;padding:12px;color:#6B3FA0}
.phead{font-weight:800;font-size:15px;display:flex;align-items:center;gap:8px;margin-bottom:10px}
.pbadge{background:#76C7C0;color:#fff;border-radius:99px;font-size:11px;font-weight:800;padding:3px 10px;margin-left:auto}
.plead{background:#fff;border:2px solid #76C7C0;border-radius:14px;padding:12px;font-size:13px;line-height:1.5}
.pnew{background:#76C7C0;color:#fff;border-radius:99px;font-size:10px;font-weight:800;padding:2px 8px}
.gold{color:#9355C1;font-weight:800}
...
.plan.hot{border-color:#76C7C0;box-shadow:0 30px 80px rgba(126,214,217,.28)}
.hotbadge{position:absolute;top:-13px;left:50%;transform:translateX(-50%);background:#76C7C0;color:#6B3FA0;font-size:11px;font-weight:800;letter-spacing:.08em;border-radius:99px;padding:5px 16px;white-space:nowrap}
.hotbadge.alt{background:#9355C1;color:#fff}
.pname{font-family:'Barlow Condensed',sans-serif;font-size:22px;font-weight:800;letter-spacing:.14em;color:#8A94A8}
.plan.hot .pname{color:#6B3FA0}
...
.qfill{height:100%;width:20%;background:#76C7C0;border-radius:99px;transition:width .3s ease}
.qstep{display:none}
.qstep.on{display:block}
.qq{font-weight:800;font-size:19px;text-align:center;margin-bottom:18px}
.opts{display:grid;grid-template-columns:1fr 1fr;gap:12px}
.opt{background:#fff;border:1.5px solid #DDE3EE;border-radius:14px;padding:18px 12px;font-weight:700;font-size:15px;color:#6B3FA0;cursor:pointer;box-shadow:0 6px 18px rgba(16,27,48,.05)}
.opt:hover{border-color:#76C7C0;background:#FFFBEF}
...
input{width:100%;padding:15px;border-radius:12px;border:1.5px solid #DDE3EE;background:#fff;color:#6B3FA0;font-size:16px;font-weight:600;margin-bottom:10px;outline:none}
input:focus{border-color:#76C7C0}
button{width:100%;padding:17px;border:none;border-radius:12px;background:#76C7C0;color:#6B3FA0;font-size:17px;font-weight:800;cursor:pointer;box-shadow:0 12px 30px rgba(248,180,8,.3)}
...
<div class="wrap">
<nav><span class="lg" style="font-family:'Barlow Condensed',sans-serif;font-weight:800;font-size:40px;letter-spacing:.5px;color:#6B3FA0">pauleza</span></nav>
<div class="hero">
  <h1>${L.h1}</h1>
  <p>${L.sub}</p>
...
app.get("/legal", (req, res) => {
  const en = req.query.lang === "en";
  const S = en ? {
    title: "Terms & Privacy — Pauleza", h: "Terms & Privacy", updated: "Last updated",
    tT: "Terms of service", t: [
      ["What Pauleza is", "Pauleza is a monthly software subscription for cleaning businesses: an app that quotes cleanings, an instant-quote widget for your page or social media, and (on the COMPLETO plan) a professional website with your brand."],
      ...
      ["Quotes are estimates", "Prices calculated by the app or widget are estimates produced from the rates each cleaning business configures. The final price of any cleaning job is always agreed between the homeowner and the cleaning business — Pauleza is the software, not a party to that job."],
    ...
    title: "Términos y Privacidad — Pauleza", h: "Términos y Privacidad", updated: "Última actualización",
    tT: "Términos del servicio", t: [
      ["Qué es Pauleza", "Pauleza es una suscripción mensual de software para negocios de limpieza: una app que cotiza limpiezas, un cotizador instantáneo para tu página o redes sociales, y (en el plan COMPLETO) una página web profesional con tu marca."],
      ...
      ["Las cotizaciones son estimados", "Los precios que calcula la app o el cotizador son estimados producidos con las tarifas que configura cada negocio de limpieza. El precio final de un trabajo siempre se acuerda entre el dueño de casa y el negocio de limpieza — Pauleza es el software, no parte de ese trabajo."],
...
a{color:#6B3FA0;font-weight:700}.top{display:flex;justify-content:space-between;align-items:center;margin-bottom:26px}
.lang{background:#6B3FA0;color:#fff;border-radius:99px;padding:8px 15px;font-weight:800;font-size:12px;text-decoration:none}</style></head><body><div class="wrap">
<div class="top"><a href="/ventas">${S.back}</a><a class="lang" href="/legal?lang=${en ? "es" : "en"}">${en ? "🇲🇽 Español" : "🇺🇸 English"}</a></div>
<h1>${S.h}</h1><p class="meta">Pauleza · ${S.updated}: 2026-07-14</p>
${sec(S.tT, S.t)}
${sec(S.pT, S.p)}
...
app.get("/bienvenida", (req, res) => {
  const en = req.query.lang === "en";
  const S = en ? {
    t: "Payment received! 🎉", h: "Welcome to Pauleza",
    ...
    t: "¡Pago recibido! 🎉", h: "Bienvenida a Pauleza",
...
h1{font-size:26px;font-weight:800}h2{font-size:17px;font-weight:800;color:#6B3FA0;margin-top:4px}
p{font-size:14.5px;color:#44506A;font-weight:500;line-height:1.6;margin-top:14px}
.cta{display:inline-block;margin-top:22px;background:#76C7C0;color:#6B3FA0;font-weight:800;font-size:15px;padding:14px 26px;border-radius:12px;text-decoration:none}
...
<div class="card">
  ...
  <h1>${S.t}</h1><h2>${S.h}</h2>
  <p>${S.p}</p><p>${S.p2}</p>
  <p style="margin-top:22px;font-weight:700;color:#6B3FA0">${S.demo}</p>
...
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Brillo Cleaning — Ejemplo Pauleza</title>
<style>
...
.ribbon{background:#76C7C0;color:#6B3FA0;text-align:center;font-weight:800;font-size:12.5px;padding:9px 14px;letter-spacing:.2px}
...
.backalto{position:fixed;bottom:18px;left:50%;transform:translateX(-50%);z-index:50;background:#6B3FA0;color:#fff;text-decoration:none;font-weight:800;font-size:14px;padding:13px 22px;border-radius:99px;box-shadow:0 14px 36px rgba(16,27,48,.5);display:flex;align-items:center;gap:8px;white-space:nowrap}
.backalto span{color:#76C7C0}
...
${embed ? "" : `<div class="ribbon">📋 PÁGINA DE EJEMPLO — imagina TU logo y TU nombre aquí. Así se vería tu página con Pauleza.</div>`}
...
<footer><b>Brillo Cleaning</b><br>Tu Ciudad, TX · Lun–Sáb 7am–7pm<br>Página de ejemplo hecha con ⚡ Pauleza — <a href="/ventas">así puede ser la tuya</a></footer>
...
.card{background:#fff;color:#6B3FA0;border-radius:26px;padding:40px 30px;max-width:440px;width:100%;text-align:center;box-shadow:0 30px 80px rgba(0,0,0,.4)}
...
.done b{color:#34A853}.wip b{color:#9355C1}
...
<p class="ft">⚡ Hecho con Pauleza</p>
</div></body></html>`);
  }
...
h1 em{color:#76C7C0;font-style:normal}
...
.colorbar button{background:#76C7C0;color:#6B3FA0;border:none;border-radius:10px;padding:10px 18px;font-weight:800;cursor:pointer}
.grid{display:flex;gap:34px;justify-content:center;flex-wrap:wrap}
.card{text-align:center}
.phone{background:#2A2352;border:9px solid #7D5AB5;border-radius:40px;padding:10px;box-shadow:0 26px 70px rgba(0,0,0,.5)}
...
.nm span{color:#76C7C0}
.ds{color:#9DA8C4;font-weight:600;font-size:13px;margin-top:4px}
.open{display:inline-block;margin-top:12px;background:#fff;color:#6B3FA0;text-decoration:none;font-weight:800;font-size:13px;padding:10px 20px;border-radius:99px}
</style></head><body>
...
<div class="colorbar">
  ...
  <input type="color" id="col" value="#6B3FA0">
  <button onclick="paint()">Pintar las 3</button>
  <button onclick="reset()" style="background:#7D5AB5;color:#fff">Colores originales</button>
</div>
<div class="grid">
${T.map(([n, nm, ds]) => `
  <div class="card">
...
.card.gold{background:linear-gradient(155deg,#16243f,#0d1729);border:none}.card.gold .v{color:#76C7C0}.card.gold .l{color:#9DA8C4}
...
.tform select:focus,.tform input:focus{border-color:#76C7C0;box-shadow:0 0 0 3px rgba(248,180,8,.18)}
.tform button{background:#76C7C0;color:#6B3FA0;border:none;border-radius:11px;padding:11px 20px;font-weight:800;cursor:pointer;white-space:nowrap}
...
.tbtn{border:1px solid #E4E7EC;background:#fff;border-radius:9px;padding:6px 11px;font-weight:700;font-size:12px;cursor:pointer;text-decoration:none;color:#6B3FA0}
.tbtn.go{background:#6B3FA0;color:#fff;border:none}.tbtn.del{color:#C5221F;border-color:#F3B4B0}
...
td a{color:#B07A00;font-weight:700;text-decoration:none}
.edit{background:#76C7C0;color:#6B3FA0 !important;border-radius:9px;padding:6px 12px;font-weight:800;font-size:12.5px}
...
.qchip:hover{border-color:#76C7C0;background:#FFFBEF}
...
.guide summary::before{content:"▸ ";color:#9355C1}
...
<div class="appheader">
  <b>pauleza · Servicio al cliente</b>
...
.row a{background:#76C7C0;color:#6B3FA0;text-decoration:none;font-weight:800;border-radius:10px;padding:9px 16px;font-size:13px}
.empty{color:#8A94A8;font-weight:600;text-align:center;padding:30px}
</style></head><body>
<header><b>pauleza · Onboarding</b></header>
<div class="wrap">
...
.sb-brand b{color:#6B3FA0;font-weight:800;font-size:15px}.sb-brand b em{color:#9355C1;font-style:normal}
...
.nav-it.on{color:#6B3FA0;background:rgba(248,180,8,.13);border-left-color:var(--gold)}
.nav-it.on .no{color:#9355C1}
...
.fcard{background:#fff;color:#2A2352;border-radius:24px;padding:24px 26px;max-width:640px;width:100%;box-shadow:0 30px 80px rgba(0,0,0,.45);margin-top:24px}
label{display:block;font-weight:600;font-size:13px;margin:16px 0 6px;color:#475067}
label:first-child{margin-top:0}
input,textarea,select{width:100%;padding:13px 15px;border-radius:13px;border:1px solid #E4E7EC;font-size:15px;font-weight:500;outline:none;font-family:inherit;color:#2A2352;background:#fff;transition:border-color .15s,box-shadow .15s}
...
.btn-dark{background:#6B3FA0;color:#fff;border:none;border-radius:11px;padding:12px 18px;font-weight:800;cursor:pointer}
...
.tphone{background:#2A2352;border:8px solid #7D5AB5;border-radius:34px;padding:7px;box-shadow:0 22px 60px rgba(0,0,0,.5)}
...
.nb-btn.next{background:var(--gold);color:#6B3FA0;border:none;box-shadow:0 8px 20px rgba(248,180,8,.3)}
.nb-btn:disabled{opacity:.35;cursor:default}
.save{width:100%;padding:16px;border:none;border-radius:14px;background:var(--gold);color:#6B3FA0;font-size:16px;font-weight:800;cursor:pointer;box-shadow:0 10px 26px rgba(248,180,8,.35);transition:transform .12s,filter .15s;margin-top:6px}
...
.rev li b{color:#475067;font-weight:600}.rev li span{font-weight:700;color:#2A2352;text-align:right}
...
.chip:has(input:checked){border-color:var(--gold);background:#FFFBEF;color:#6B3FA0}
.chip:has(input:checked)::before{content:"✓";color:#9355C1;font-weight:900}
...
<div class="layout">
<aside>
  <div class="sb-brand"><b>pauleza</b></div>
...
<main>
  <div class="mtop"><b style="color:#fff;font-weight:800;font-size:15px">pauleza</b><div><div class="mstep" id="mstep">Paso 1 de 7</div><div class="mtitle" id="mtitle">Bienvenida</div></div></div>
  <div class="stage">
    ...
    <section class="slide">
      <div class="s-in top">
        ...
        <div class="fcard">
          ...
          <p style="text-align:center;color:#475067;font-weight:600;font-size:14px;margin:8px 0 18px;line-height:1.6">Revisa que todo esté bien. Al enviar, nuestro equipo de diseño arma tu página a mano y te la entregamos lista en <b style="color:#2A2352">10–14 días</b>.</p>
          <ul class="rev">
          ...
          <div id="staff" style="display:${st.template || st.about ? "block" : "none"};margin-top:14px;text-align:center">
            ...
            <button onclick="publish()" id="pub" class="btn-dark" style="background:${st.published ? "#1E7B3C" : "#6B3FA0"}">${st.published ? "✓ Publicada — clic para ocultar" : "🚀 Publicar página al cliente"}</button>
          </div>
        </div>
      </div>
    </section>
  ...
  fetch('/api/onboarding/publish?key=${K}',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({slug:${JSON.stringify(c.slug)},publish:!PUBLISHED})})
    .then(function(r){return r.json();}).then(function(j){
      ...
      if(j&&j.ok){PUBLISHED=j.published;
        ...
        pub.style.background=PUBLISHED?'#1E7B3C':'#6B3FA0';
      }
...
app.post("/api/onboarding/save", async (req, res) => {
  ...
  data.site = {
    ...
    color: /^#?[a-f0-9]{6}$/i.test(String(b.color || "")) ? (String(b.color).startsWith("#") ? b.color : "#" + b.color) : (data.site?.color || "#6B3FA0"),
...
.iphone{position:relative;background:#2A2352;border:9px solid #7D5AB5;border-radius:44px;padding:10px;box-shadow:0 30px 80px rgba(0,0,0,.55)}
.inotch{position:absolute;top:10px;left:50%;transform:translateX(-50%);width:100px;height:20px;background:#7D5AB5;border-radius:0 0 12px 12px;z-index:2}
...
<div class="layout">
<aside id="sb"><div class="sb-brand"><b style="color:#6B3FA0;font-weight:900;font-size:22px">pauleza</b></div><div class="sb-label">MAID FLOW</div><nav id="nav"></nav><div class="sb-foot">Presentación del rol</div></aside>
<div class="scrim" id="scrim" onclick="sb(false)"></div>
<main>
<div class="mtop"><button onclick="sb(true)">☰ Menú</button><b style="font-weight:800">pauleza</b><span style="width:64px"></span></div>
<div class="stage" id="stage">

<section class="slide" data-t="El rol">
  <div class="s-bg" style="background:linear-gradient(160deg,#062B22,#6B3FA0)"></div><div class="s-veil"></div>
  <div class="s-in">
...
<section class="slide" data-t="Qué vendemos">
  ...
  <div class="s-in">
    ...
    <div class="grid">
      ...
      <div class="card"><div class="ic">📲</div><h3>La app Pauleza</h3><p>Cotiza limpiezas, manda la cotización, recibe los leads.</p></div>
...
<section class="slide" data-t="Tu rol: Contenido">
  ...
  <div class="s-in" style="max-width:1000px">
    ...
    <div class="grid">
      ...
      <div class="card"><div class="ic">📸</div><h3>Fotos del equipo</h3><p>Tú y el equipo con la camisa Pauleza, profesionales.</p></div>
    </div>
  </div>
</section>
...
<section class="slide" data-t="Empecemos">
  ...
  <div class="s-in">
    ...
    <ul class="pts">
      ...
      <li><b>3</b> Agenda la foto del equipo con la camisa Pauleza</li>
    </ul>
...
<style>body{font-family:Arial;max-width:560px;margin:40px auto;padding:0 16px;color:#6B3FA0}h2{margin-bottom:6px}
.link{background:#F1ECFA;border:2px solid #76C7C0;border-radius:12px;padding:13px;word-break:break-all;font-size:14px;margin:10px 0;display:flex;gap:10px;align-items:center}
.link button{margin-left:auto;background:#76C7C0;color:#6B3FA0;border:none;border-radius:8px;padding:8px 14px;font-weight:800;cursor:pointer;flex-shrink:0}
...
${paid
  ...
  : `<p style="background:#F1ECFA;border:1.5px solid #76C7C0;color:#7A5A00;border-radius:12px;padding:10px 14px;font-weight:700">⏳ El link de acceso se ACTIVA solo cuando Stripe confirme su pago (≈1 min después de pagar). Si pagó por otro medio, el admin la activa desde su tablero.</p>`}
...
<div class="link"><span><b>🎨 Onboarding de su página</b></span><a href="/onboarding?key=${K}&slug=${c.slug}" style="margin-left:auto;background:#76C7C0;color:#6B3FA0;border-radius:8px;padding:8px 14px;font-weight:800;text-decoration:none">Abrir →</a></div>
...
app.get("/closer", async (req, res) => {
  ...
  const welcome = en
    ? `Congratulations and welcome to Pauleza! 🎉 Tap this link from your phone and save it — it's your personal key to your app: [PASTE THEIR ACCESS LINK HERE]. You can quote cleanings and send quotes starting today. See you at your onboarding call 💪`
    : `¡Felicidades y bienvenido a Pauleza! 🎉 Toca este link desde tu teléfono y guárdalo — es tu llave personal a tu app: [PEGA AQUÍ SU LINK DE ACCESO]. Hoy mismo puedes cotizar limpiezas y mandar cotizaciones. Nos vemos en tu llamada de onboarding 💪`;
  ...
  const L = en ? {
    ...
    obj: [
      ...
      ["\"I'm not good with technology\"", "“If you can send a WhatsApp, you can use Pauleza. We do the onboarding with you, step by step.”"],
    ...
    obj: [
      ...
      ["\"No soy bueno con la tecnología\"", "“Si sabes mandar un WhatsApp, sabes usar Pauleza. El onboarding lo hacemos contigo, paso a paso.”"],
...
.alta input:focus{border-color:#76C7C0;box-shadow:0 0 0 4px rgba(248,180,8,.18)}
.alta button{background:#76C7C0;color:#6B3FA0;border:none;border-radius:13px;padding:13px 24px;font-weight:700;cursor:pointer;font-size:14.5px;box-shadow:0 6px 16px rgba(248,180,8,.3);transition:transform .12s,filter .15s}
.alta button:hover{filter:brightness(1.03)}.alta button:active{transform:scale(.97)}
ol{padding-left:22px;margin-top:4px}ol li{margin-bottom:10px;font-weight:500;color:#7D5AB5}
ul{padding-left:22px}ul li{margin-bottom:7px;font-weight:500;color:#7D5AB5}
small{color:#9097A3}
.sc{background:#fff;border:1px solid rgba(16,27,48,.05);border-left:3px solid #76C7C0;border-radius:16px;padding:16px 18px;margin:10px 0;box-shadow:0 1px 2px rgba(16,27,48,.04),0 8px 22px rgba(16,27,48,.045)}
...
.sc p{font-size:15px;font-style:italic;color:#7D5AB5;line-height:1.6}
...
.link button{background:#76C7C0;color:#6B3FA0;border:none;border-radius:11px;padding:9px 15px;font-weight:700;cursor:pointer;flex-shrink:0;font-size:13px;transition:filter .15s}
...
.ob b{font-size:14px;font-weight:700;color:#2A2352}
...
.navbtn{display:inline-flex;align-items:center;gap:8px;background:#fff;border:1px solid rgba(16,27,48,.06);border-radius:14px;padding:13px 20px;font-weight:700;font-size:14.5px;color:#2A2352;text-decoration:none;box-shadow:0 1px 2px rgba(16,27,48,.04),0 8px 22px rgba(16,27,48,.05);transition:transform .15s,box-shadow .15s}
.navbtn:hover{transform:translateY(-1px);box-shadow:0 2px 4px rgba(16,27,48,.06),0 14px 30px rgba(16,27,48,.09)}
.navbtn.primary{background:#76C7C0;border:none;box-shadow:0 6px 18px rgba(248,180,8,.35)}
...
.card.gold .v{color:#76C7C0}.card.gold .l{color:#9DA8C4}
...
.mform input:focus{border-color:#76C7C0;box-shadow:0 0 0 4px rgba(248,180,8,.18)}
.mform button{background:#6B3FA0;color:#fff;border:none;border-radius:12px;padding:12px 20px;font-weight:700;cursor:pointer;font-size:14px}
...
.mbtn.show{background:#E7F7ED;color:#10803C}.mbtn.no{background:#FDECEC;color:#C5221F}.mbtn.win{background:#76C7C0;color:#6B3FA0}
...
.mnote{flex-basis:100%;font-family:inherit;margin-top:4px;padding:9px 12px;border-radius:10px;border:1px solid #E4E7EC;font-size:13px;font-weight:500;color:#7D5AB5;outline:none;transition:border-color .2s,box-shadow .15s}
.mnote::placeholder{color:#B6BCC8;font-weight:500}
.mnote:focus{border-color:#76C7C0;box-shadow:0 0 0 3px rgba(248,180,8,.16)}
...
<div class="cols">
  <div class="col">
    ...
    ${[["complete", "COMPLETO", "$249"], ["widget", "WIDGET", "$149"], ["pro", "PRO", "$49"]].map(([plan, label, price]) => {
      ...
      return link
        ? `<div class="link"><span><b>💳 ${label} — ${price}/mes</b><br><small>${esc(link)}</small></span><a href="${link}" target="_blank" rel="noreferrer" style="background:#6B3FA0;color:#fff;border-radius:11px;padding:9px 15px;font-weight:700;text-decoration:none;flex-shrink:0;font-size:13px">${L.open}</a><button onclick="cp(this,'${link}')">${L.copy}</button></div>`
...
app.get("/cierre", (req, res) => {
  ...
  const welcome = `¡Felicidades y bienvenido a Pauleza! 🎉 Toca este link desde tu teléfono y guárdalo — es tu llave personal a tu app: [PEGA AQUÍ SU LINK DE ACCESO]. Hoy mismo puedes cotizar limpiezas y mandar cotizaciones. Nos vemos en tu llamada de onboarding 💪`;
  const esc = (s) => String(s).replace(/</g, "&lt;");
  res.send(`<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Pauleza · Cierre (privado)</title><style>
body{font-family:Arial;max-width:640px;margin:30px auto;padding:0 18px;color:#6B3FA0;line-height:1.55}
...
.link{background:#F1ECFA;border:2px solid #76C7C0;border-radius:12px;padding:12px;word-break:break-all;font-size:14px;margin:8px 0;display:flex;gap:10px;align-items:center}
.link button{margin-left:auto;background:#76C7C0;color:#6B3FA0;border:none;border-radius:8px;padding:8px 14px;font-weight:800;cursor:pointer;flex-shrink:0}
.link small{color:#67718A}
ol li{margin-bottom:10px}small{color:#67718A}
</style></head><body>
<h1>🔒 Cierre · pauleza</h1>
...
<b>"No soy bueno con la tecnología"</b> → "Por eso lo hicimos así: si sabes mandar un WhatsApp, sabes usar Pauleza. Y el onboarding lo hacemos contigo, en español, paso a paso. No estás solo."<br><br>
...
app.get("/demo", (req, res) => {
  ...
  const L = en ? {
    title: "Pauleza · Presentation", presentation: "PRESENTATION", forClients: "Client presentation",
    ...
    p2a: "Operator-founder: he runs real service businesses, not just software", p2b: "20+ people on the Pauleza team working behind your account", p2c: "We use our own tools, every single day",
    cap2: "Rolando · Founder of Pauleza", ph2a: "Photo of Rolando and the team", ph2b: "in Pauleza shirts",
    ...
    title: "Pauleza · Presentación", presentation: "PRESENTACIÓN", forClients: "Presentación para clientes",
    ...
    p2a: "Fundador emprendedor: tiene negocios de servicios reales, no solo software", p2b: "Más de 20 personas del equipo Pauleza trabajando detrás de tu cuenta", p2c: "Usamos nuestras propias herramientas, todos los días",
    cap2: "Rolando · Fundador de Pauleza", ph2a: "Foto de Rolando y el equipo", ph2b: "con la camisa Pauleza",
...
.iphone{position:relative;background:#2A2352;border:10px solid #7D5AB5;border-radius:48px;padding:11px;box-shadow:0 30px 80px rgba(0,0,0,.55)}
.inotch{position:absolute;top:11px;left:50%;transform:translateX(-50%);width:110px;height:22px;background:#7D5AB5;border-radius:0 0 13px 13px;z-index:2}
...
.pcard{background:#fff;color:#6B3FA0;border-radius:26px;padding:34px 32px;text-align:center;box-shadow:0 34px 90px rgba(248,180,8,.18),0 30px 70px rgba(0,0,0,.45);width:min(340px,100%)}
.pcard .pk{color:#9355C1;font-weight:800;font-size:11px;letter-spacing:2.5px}
...
.bub.me{background:#6B3FA0;color:#fff;margin-left:auto;border-bottom-right-radius:5px}
...
.ch-in input:focus{border-color:#76C7C0}
.ch-in button{background:#76C7C0;color:#6B3FA0;border:none;border-radius:11px;padding:0 18px;font-weight:800;font-size:17px;cursor:pointer}
...
.bbar .next{background:var(--gold);color:var(--navy);border:none}
.bbar .ct{font-family:'Fraunces',Georgia,serif;font-size:15px;color:var(--mut)}
.langpill{position:fixed;top:16px;right:18px;z-index:45;background:rgba(255,255,255,.95);color:#6B3FA0;border-radius:99px;padding:9px 18px;font-weight:800;font-size:13px;text-decoration:none;box-shadow:0 10px 28px rgba(0,0,0,.35)}
...
#ckit{display:none;position:fixed;right:18px;bottom:70px;z-index:80;background:#fff;color:#6B3FA0;border-radius:16px;padding:14px 16px;box-shadow:0 24px 60px rgba(0,0,0,.5);width:280px}
...
#ckit button{background:#76C7C0;color:#6B3FA0;border:none;border-radius:8px;padding:6px 12px;font-weight:800;font-size:12px;cursor:pointer}
...
<div class="layout">
<aside id="sb">
  <div class="sb-brand"><b style="color:#6B3FA0;font-weight:900;font-size:24px">pauleza</b></div>
...
<main>
<div class="mtop"><button class="mt-b" onclick="toggleSb(true)">${L.menu}</button><b>pauleza</b><span style="width:64px"></span></div>
<div class="stage" id="stage">
...
<section class="slide" data-t="${L.t3}">
  ...
  <div class="s-in" style="max-width:1150px">
    ...
    <p class="body" style="margin-top:28px;font-size:clamp(17px,2.3vw,23px);max-width:940px"><b style="color:#76C7C0">${L.c3}</b></p>
  </div>
</section>
...
app.get("/i", (req, res) => {
  ...
  const es = d.lang !== "en";
  const L = es
    ? { inv: "FACTURA", est: "COTIZACIÓN", for: "Preparado para", item: "Concepto", subtotal: "Subtotal", deposit: "Depósito recibido", due: "SALDO PENDIENTE", paid: "PAGADO", how: "CÓMO PAGAR", zelle: "Zelle", cash: "Efectivo o cheque aceptado", print: "🖨️ Imprimir / Guardar PDF", made: "Hecho con Pauleza", meas: "Detalles de la limpieza", date: "Fecha", area: "Área a limpiar", pitch: "Tipo de limpieza", sqs: "Recámaras / baños", imgOf: "Referencia", valid: "Esta cotización es válida por 30 días.", sig: "Autorizado por (firma del cliente)", sigDate: "Fecha" }
    : { inv: "INVOICE", est: "QUOTE", for: "Prepared for", item: "Item", subtotal: "Subtotal", deposit: "Deposit received", due: "BALANCE DUE", paid: "PAID", how: "HOW TO PAY", zelle: "Zelle", cash: "Cash or check accepted", print: "🖨️ Print / Save PDF", made: "Made with Pauleza", meas: "Cleaning details", date: "Date", area: "Area to clean", pitch: "Cleaning type", sqs: "Bedrooms / baths", imgOf: "Reference", valid: "This quote is valid for 30 days.", sig: "Authorized by (client signature)", sigDate: "Date" };
...
<style>
  body{margin:0;font-family:-apple-system,'Segoe UI',Roboto,sans-serif;background:#F4F5F7;color:#6B3FA0}
  .page{max-width:560px;margin:0 auto;background:#fff;min-height:100vh}
  .hd{background:#6B3FA0;color:#fff;padding:22px 24px}
  .hd .biz{font-size:22px;font-weight:800;letter-spacing:.02em}
  .hd .sub{color:#9DA8C4;font-size:13px;margin-top:2px}
  .tag{display:inline-block;background:#76C7C0;color:#fff;font-size:12px;font-weight:800;border-radius:99px;padding:3px 12px;margin-top:10px;letter-spacing:.06em}
  ...
  .due .amt{color:${d.paid ? "#1E9E5A" : "#76C7C0"}}
  .pay{background:#F1ECFA;border-radius:12px;padding:14px 16px;font-size:15px}
  .pay b{display:block;font-size:11px;letter-spacing:.1em;color:#76C7C0;margin-bottom:6px}
  .btn{display:block;width:calc(100% - 48px);margin:18px 24px;background:#76C7C0;color:#fff;border:none;border-radius:12px;padding:15px;font-size:16px;font-weight:800;cursor:pointer}
...
<div style="display:flex;gap:24px;margin-top:34px">
  <div style="flex:2;border-top:1.5px solid #6B3FA0;padding-top:5px">${L.sig}</div>
  <div style="flex:1;border-top:1.5px solid #6B3FA0;padding-top:5px">${L.sigDate}</div>
</div></div>` : ""}
...
await ensureAccount("alto-ventas", "Pauleza Ventas", { biz: "Pauleza", lang: "es" });

app.listen(PORT, () => {
  console.log(`Pauleza server on http://localhost:${PORT}`);
