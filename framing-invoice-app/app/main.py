...
def _load_baseline(conn, account_number=None) -> Baseline:
    """Load the baseline. If account_number is given, scope to that account's
    itemized price list so a quote only matches against the right account."""
    if account_number:
        rows = conn.execute("SELECT * FROM baseline_items WHERE gl_account=?",
...
@app.post("/api/coa/{account_number}/baseline")
async def coa_attach_baseline(account_number: str, file: UploadFile = File(...)):
    """Attach an itemized price list to an account: upload an Excel/CSV with
    Item #, Description, UM, Unit Price → baseline items coded to this account.
    Replaces this account's existing items (other accounts are untouched)."""
    acct_no = str(account_number).strip()
    with db.connect() as conn:
        acct = conn.execute(
    ...
    dest = os.path.join(UPLOAD_DIR, f"baseline_{acct_no}_{uuid.uuid4().hex}_{file.filename}")
    with open(dest, "wb") as f:
        f.write(await file.read())
    try:
        from .importer import import_price_list
        count = import_price_list(dest, gl_account=acct_no, department=department)
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Import failed: {e}")
    return {"account_number": acct_no, "imported": count}


@app.get("/api/coa/{account_number}/baseline")
def coa_account_baseline(account_number: str):
    """List the itemized baseline items attached to an account."""
    with db.connect() as conn:
        rows = conn.execute(
            "SELECT * FROM baseline_items WHERE gl_account=? ORDER BY item_number",
            (str(account_number).strip(),)).fetchall()
...
def compare_endpoint(payload: dict = Body(...)):
    ...
    # When the user picked an account on the home screen, scope the baseline to
    # that account's itemized price list and tag everything to it.
    account_number = str(inv.get("account_number") or "").strip() or None
    ...
    with db.connect() as conn:
        ...
        baseline = _load_baseline(conn, account_number)
        allowed, by_cat = _variance_settings(conn)
        ...
        out_rows = []
        for r in results:
            if account_number:  # user explicitly chose the account -> tag all lines
                dept = acct_row["department"] if acct_row else None
                gl = account_number
            else:
...
def dashboard():
    ...
    with db.connect() as conn:
        ...
        # Itemized baseline coverage per account (how many accounts have a price
        # list attached vs. still need one).
        baseline_coverage = [dict(r) for r in conn.execute(
            """SELECT coa.account_number, coa.account_name, coa.department,
                      COUNT(b.id) AS items
               FROM chart_of_accounts coa
               LEFT JOIN baseline_items b ON b.gl_account = coa.account_number
               GROUP BY coa.account_number
               ORDER BY items DESC, coa.account_number"""
        ).fetchall()]
        accounts_with_baseline = sum(1 for r in baseline_coverage if r["items"] > 0)
        ...
        return {
            "kpis": {
                ...
                "accounts_total": len(baseline_coverage),
                "accounts_with_baseline": accounts_with_baseline,
            },
