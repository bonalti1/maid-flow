...
async function load() {
  ...
  const k = d.kpis;
  $("#kpis").innerHTML = `
    <div class="kpi card"><div class="num">${k.invoice_count}</div><div class="lbl">Quotes checked</div></div>
    <div class="kpi card"><div class="num">${money(k.total_spend)}</div><div class="lbl">Total checked</div></div>
    <div class="kpi over card"><div class="num">${money(k.total_overcharge)}</div><div class="lbl">Overcharges caught</div></div>
    <div class="kpi pending card"><div class="num">${k.accounts_with_baseline||0}/${k.accounts_total||0}</div><div class="lbl">Accounts with a price list</div></div>`;

  // Baseline coverage — which accounts have an itemized price list
  const bc = d.baseline_coverage || [];
  $("#baselineCoverage").innerHTML = bc.length ? `<table class="tbl"><thead><tr>
      <th>Account</th><th>Phase</th><th>Price list</th></tr></thead><tbody>` +
    bc.map((x) => `<tr>
      <td>${esc(x.account_number)} · ${esc(x.account_name)}</td>
      <td>${esc(x.department)}</td>
      <td>${x.items > 0 ? '<span class="pill ok">'+x.items+' items</span>' : '<span class="pill review">none yet</span>'}</td>
    </tr>`).join("") + "</tbody></table>"
